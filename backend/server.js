...
app.post('/api/book-consultation', [
  ...
  body('time').matches(/^\d{1,2}:\d{2}\s?(AM|PM)$/i).withMessage('Invalid time format'),
  ...
  try {
    ...
    // Parse date and time
    const meetingDateTime = parseDateTime(bookingData.date, bookingData.time, bookingData.timezone);
    const endTime = new Date(meetingDateTime.getTime() + 60 * 60 * 1000); // 1 hour meeting

    // Create Google Meet link and calendar event
    const calendarEvent = await createCalendarEvent(bookingData, meetingDateTime, endTime);
...
Message: ${bookingData.message || 'N/A'}
    ...
    attendees: [
      { email: process.env.ADMIN_EMAIL, organizer: true, responseStatus: 'accepted' },
